...
const SettingsIcon = () => (
    <Link to="/settings">
        <svg width="20px" height="20px" viewBox="0 0 20 20" fill="white" xmlns="http://www.w3.org/2000/svg">
...
const NavigationButton = ({ direction, onClick }: NavigationButtonProps) => {
    return (
        <button
            ...
            onClick={onClick}
            aria-label={LabelsMap[direction]}
        >
            {IconsMap[direction]}
        </button>
    );
...
const DateDisplay = ({ date }: { date: Date }) => {
    return (
        <span className="text-sm font-bold" data-tauri-drag-region>
...
export const BottomBar = () => {
    ...
    const { selectedDate, goToPreviousDay, goToNextDay, isToday } = useDateNavigation();

    if (isSettingsPage) {
        return <div className="flex justify-between items-center text-muted-foreground p-0.5 cursor-grab" data-tauri-drag-region>
            <HomeIcon />
        </div>
    }

    return (
        <div className="border-t border-t-gray-600 flex justify-between items-center text-muted-foreground p-0.5 cursor-grab" data-tauri-drag-region>
            {
                isToday()
                    ? <NavigationButton direction="settings" onClick={console.log} />
                    : <NavigationButton direction="left" onClick={goToPreviousDay} />
            }
